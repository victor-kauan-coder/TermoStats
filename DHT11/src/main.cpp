/*
  ============================================================================
  Nó IoT de Temperatura e Umidade — ESP32 + DHT11 (módulo)
  ----------------------------------------------------------------------------
  ...
  Construído sobre os conceitos de:
    ...
    Aula 06 — barramentos: o DHT11 já traz o conversor A/D embutido
  ...
  - Lê temperatura e umidade de um DHT11 em módulo (sensor digital de fábrica —
  ...
  - Módulo DHT11, ALIMENTADO EM 3V3. O módulo já possui o resistor de
    pull-up do pino de dado; não é necessário adicionar outro externamente.
  ...
  - GPIO4  (rótulo A1) — dado do módulo DHT11 (uso digital; não entra em conflito
...
// ---------------------------------------------------------------------------
#define DHT_PIN         4        // GPIO4 — dado do módulo DHT11, digital
...
// ---------------------------------------------------------------------------
// TESTE ACELERADO DE 100 CICLOS
// Mantenha em 1 apenas durante a coleta. O modo ignora temporariamente os
// valores gravados na NVS, preserva as 5 amostras por ciclo e para sozinho
// após a centésima leitura. Ao terminar, volte para 0 e grave o firmware
// normal novamente.
// ---------------------------------------------------------------------------
#define TESTE_100_CICLOS      1
#define TOTAL_CICLOS_TESTE    100
#define INTERVALO_TESTE_S     1
...
#define I_DHT11_ATIVO_mA          2.5f     // origem: folha de dados do DHT11; confirmar na bancada
...
void setup() {
  ...
  Serial.println();
    Serial.printf("== No DHT11 - despertar #%lu ==\n", (unsigned long)bootCount);
...
#else
  Serial.printf("MODO TESTE: ciclo %lu de %d; 5 amostras; sono de %d s.\n",
                (unsigned long)bootCount, TOTAL_CICLOS_TESTE, INTERVALO_TESTE_S);
#endif
  ...
  if (leituraOk) {
    ...
    Serial.println("Precisao tipica DHT11: +-2 C / +-5% RH; resolucao de 1 C e 1% RH");
    if (!isnan(vbat)) {
    ...
    falhasConsecutivas++;
    Serial.printf("Falha de leitura do DHT11. Falhas consecutivas: %lu\n",
  ...
  if (bootCount == 1 && !TESTE_100_CICLOS) {
  ...
  #if TESTE_100_CICLOS
  if (bootCount >= TOTAL_CICLOS_TESTE) {
    Serial.println("==== TESTE DE 100 CICLOS CONCLUIDO ====");
    Serial.println("ESP32 permanecera acordada; copie o log do monitor serial.");
    digitalWrite(LED_STATUS_PIN, LOW);
    digitalWrite(BUZZER_PIN, LOW);
    digitalWrite(ALARME_LED_PIN, LOW);
    while (true) {
      delay(1000); // mantém o watchdog alimentado após o fim do teste
    }
  }
  #endif
...
#if TESTE_100_CICLOS
  // Sobrescreve somente a configuração usada nesta execução; não grava na
  // NVS. Assim o firmware normal continuará com seus parâmetros originais.
  cfg.intervaloNormalS = INTERVALO_TESTE_S;
  cfg.intervaloAlarmeS = INTERVALO_TESTE_S;
  cfg.tempMinC         = 0.0f;
  cfg.tempMaxC         = 50.0f;
  cfg.umidMinPct       = 0.0f;
  cfg.umidMaxPct       = 100.0f;
#endif
...
void talvezEntrarModoConfig() {
  ...
  while (millis() - inicio < CONFIG_JANELA_MS) {
    ...
    delay(1); // entrega tempo ao RTOS/watchdog enquanto aguarda a serial
  }
}
...
void imprimirOrcamentoEnergia(bool emAlarme) {
  ...
  float cargaAcordadoMah = (I_ACORDADO_SEM_RADIO_mA + I_DHT11_ATIVO_mA)
